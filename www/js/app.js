...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (window.Capacitor?.Plugins?.App) {
    window.Capacitor.Plugins.App.addListener('backButton', () => {
      ...
      if (activeId === 'screen-settings') {
        showScreen('screen-library');
        Library.render();
        return;
      }
      if (activeId === 'screen-processing') {
        // برخلافِ صفحه‌های دیگه، اینجا یه پردازشِ در حالِ اجرا ممکنه پشتش باشه —
        // به‌جای برگشتِ بی‌صدا (که کاربر فکر می‌کنه خارج شده ولی درخواست‌ها همچنان
        // ادامه دارن)، صریح می‌پرسیم. «نه» یعنی همینجا بمون، پردازش دست‌نخورده ادامه پیدا کنه.
        if (confirm('پردازش متوقف بشه و به کتابخونه برگردی؟')) {
          BookImport.pauseActiveImport();
          showScreen('screen-library');
          Library.render();
        }
        return;
