const Reader = (() => {
  ...
  let isProgrammaticScroll = false; // جلوگیری از تداخل snap با ناوبری برنامه‌ای
  ...
  async function open(bookId) {
    ...
    // باید صبر کنیم تا مرورگر CSS columns رو کامل layout کنه
    // این فرآیند چند frame طول می‌کشه
  ...
  // ===== صبر برای کامل شدن layout =====
  // مرورگر CSS columns رو async layout می‌کنه
  // این تابع صبر می‌کنه تا scrollWidth واقعی باشه
  async function waitForLayout(element) {
    return new Promise(resolve => {
      ...
      const check = () => {
        ...
        if (w === lastWidth) {
          stableCount++;
          if (stableCount >= 3) { resolve(); return; }
        } else {
          stableCount = 0;
          lastWidth = w;
        }
  ...
  function bindEvents() {
    ...
    // touch روی content برای swipe
    content.addEventListener('touchstart', onTouchStart, { passive: true });
    content.addEventListener('touchend', onTouchEnd, { passive: true });

    // کلیک روی کل screen-reader (نه فقط content) تا لبه‌ها هم کار کنن
    screen.addEventListener('click', onScreenClick);

    // scroll listener برای sync کردن currentColumn با موقعیت واقعی
    content.addEventListener('scroll', onScroll, { passive: true });

    document.getElementById('btn-reader-menu').onclick = (e) => {
      e.stopPropagation();
      hideBars();
      showSettingsPanel();
    };
    document.getElementById('btn-back-from-reader').onclick = (e) => {
      e.stopPropagation();
      Reader.close();
  ...
  // sync currentColumn بعد از هر scroll (مثلاً وقتی snap اتفاق می‌افته)
  function onScroll() {
    ...
    window._scrollSyncTimer = setTimeout(() => {
      const content = document.getElementById('reader-content');
      if (!content) return;
      const nearest = Math.round(content.scrollLeft / window.innerWidth);
  ...
  function onTouchEnd(e) {
    ...
    e.preventDefault();
    if (dx < 0) nextPage();  // کشیدن به چپ = صفحه بعد
    else prevPage();          // کشیدن به راست = صفحه قبل
  }

  // کلیک روی screen-reader (شامل حاشیه‌های چپ و راست)
  function onScreenClick(e) {
    // اگه کلیک روی دکمه‌های نوار بود، نادیده بگیر
    if (e.target.closest('#reader-top-bar') || e.target.closest('#reader-bottom-bar')) return;
    // اگه پانل تنظیمات باز بود، ببندش
    const panel = document.getElementById('reader-settings-panel');
    if (panel && panel.style.display !== 'none') { hideSettingsPanel(); return; }

    const w = window.innerWidth;
    const x = e.clientX;
    // ناحیه چپ (20%): صفحه بعد در RTL
    if (x < w * 0.2) { nextPage(); return; }
    // ناحیه راست (20%): صفحه قبل در RTL
    if (x > w * 0.8) { prevPage(); return; }
    // وسط: نمایش/مخفی نوارها
    toggleBars();
  }
  ...
  function close() {
    ...
    currentBook = null; totalColumns = 0; currentColumn = 0; isProgrammaticScroll = false;
    const content = document.getElementById('reader-content');
    const screen = document.getElementById('screen-reader');
