const Reader = (() => {
  ...
  async function open(bookId) {
    ...
    // تشخیص: چاپ مقادیر واقعی برای debugging
    const dbg = document.getElementById('reader-content');
    const cs = window.getComputedStyle(dbg);
    alert(
      `scrollWidth: ${dbg.scrollWidth}\n` +
      `clientWidth: ${dbg.clientWidth}\n` +
      `scrollHeight: ${dbg.scrollHeight}\n` +
      `clientHeight: ${dbg.clientHeight}\n` +
      `computed height: ${cs.height}\n` +
      `computed column-width: ${cs.columnWidth}\n` +
      `computed overflow-x: ${cs.overflowX}\n` +
