const Reader = (() => {
  ...
  function onScreenClick(e) {
    ...
    if (e.target.closest('#footnote-popup')) return; // خودِ پاپ‌آپِ پانویس جدا مدیریت می‌شه
    const fnrefEl = e.target.closest('sup.fnref');
    if (fnrefEl) { Highlights.hidePopup(); Highlights.closeManagePanel(); showFootnotePopup(fnrefEl); return; }
    const markEl = e.target.closest('mark.highlight');
    if (markEl) { hideFootnotePopup(); Highlights.openManagePanel(markEl); return; }
    const inner = document.getElementById('reader-inner');
    ...
    hideFootnotePopup(); // تپ جای دیگه = بستنِ پاپ‌آپِ پانویسِ احتمالاً بازمونده
  ...
  // ===== پاپ‌آپِ پانویس — تپ رو sup.fnref =====
  // همون الگوی پوزیشنینگِ .highlight-popup تو highlights.js (اندازه‌گیریِ ابعادِ واقعی
  // با visibility:hidden قبل از نمایش، به‌جای یه عددِ حدسی)، ولی بدونِ فاصله‌ی اطمینانِ
  // منوی native اندروید — چون این یه تپِ سادست، نه یه drag-selection که منوی
  // Copy/Share/Select all رو باز کنه.
  function showFootnotePopup(fnrefEl) {
    const popup = document.getElementById('footnote-popup');
    const textEl = document.getElementById('footnote-popup-text');
    if (!popup || !textEl || !currentBook) return;

    const fnId = fnrefEl.dataset.fn;
    const bodyHtml = (currentBook.footnotes || {})[fnId];
    textEl.innerHTML = bodyHtml
      ? bodyHtml
      : '<span class="footnote-missing">این پانویس در این نسخه در دسترس نیست</span>';

    const rect = fnrefEl.getBoundingClientRect();

    popup.style.visibility = 'hidden';
    popup.style.display = 'flex';
    const popupRect = popup.getBoundingClientRect();
    const popupW = popupRect.width || 220;
    ...
    if (top < 8) top = rect.bottom + 10; // اگه بالای صفحه جا نبود، پایینِ ارجاع نشونش بده
  ...
  function hideFootnotePopup() {
    const popup = document.getElementById('footnote-popup');
    if (popup && popup.style.display !== 'none') { popup.style.display = 'none'; return true; }
    return false;
  }
