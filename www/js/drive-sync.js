const DriveSync = (() => {
  ...
  // ===== لیست همه‌ی فایل‌های داخل پوشه‌ی بکاپ — پایه‌ی مشترک برای کتاب‌ها و یادداشت‌ها =====
  async function listRemoteFiles() {
    ...
    return (data.files || []).map(f => ({
      ...
      // فایل‌های کتابِ قدیمی این تگ رو ندارن — نبودش یعنی «کتاب»، نه یادداشت
      dataType: f.appProperties && f.appProperties.dataType ? f.appProperties.dataType : 'book',
    }));
  }

  // ===== لیست فایل‌های داخل پوشه‌ی بکاپ =====
  async function listRemoteBooks() {
    return (await listRemoteFiles()).filter(f => f.dataType === 'book');
  }

  async function listRemoteAnnotationFiles() {
    return (await listRemoteFiles()).filter(f => f.dataType === 'annotations');
  }
  ...
  // ===== آپلود یادداشت‌ها/هایلایت‌های یک کتاب — فایل جدا و کوچیک، جدا از محتوای کتاب =====
  // چرا فایل جدا: annotation ها مدام عوض می‌شن (هر هایلایت/یادداشت/رنگ)، ولی محتوای
  // کتاب یک‌بار برای همیشه ساخته می‌شه. اگه تو همون فایل کتاب می‌ذاشتیم، هر تغییرِ
  // کوچیکِ یادداشت یعنی آپلود دوباره‌ی کل محتوای کتاب (تا ۴ مگابایت) — به‌جاش همیشه
  // کل لیستِ annotation های همون کتاب رو (که خودش خیلی کوچیکه) کامل جایگزین می‌کنیم؛
  // چون فایل کوچیکه، نیازی به ردیابیِ دقیقِ «چی عوض شده» نیست، upload اضافیِ
  // گاه‌به‌گاه کاملاً بی‌ضرره.
  async function uploadAnnotations(bookId, bookTitle, annotationsList) {
    const contentStr = JSON.stringify(annotationsList);
    const byteSize = new Blob([contentStr]).size;
    if (byteSize > MAX_SAFE_BYTES) {
      throw new Error(`یادداشت‌های «${bookTitle}» برای بکاپ خیلی حجیمن.`);
    }

    const fid = await ensureFolder();
    const existingFiles = await listRemoteAnnotationFiles();
    const existing = existingFiles.find(f => f.bookId === bookId);

    const fileName = sanitizeFileName(bookTitle) + ' - یادداشت‌ها.json';
    const metadata = existing
      ? { name: fileName }
      : { name: fileName, parents: [fid], appProperties: { bookId, dataType: 'annotations' } };

    const { body, boundary } = buildMultipartBody(metadata, contentStr);
    const url = existing
      ? `${UPLOAD_API}/files/${existing.driveFileId}?uploadType=multipart`
      : `${UPLOAD_API}/files?uploadType=multipart`;
  ...
  // ===== دانلود یادداشت‌های یک کتاب از Drive =====
  async function downloadAnnotations(driveFileId) {
    const resp = await driveFetch(`${API}/files/${driveFileId}?alt=media`);
    const text = await resp.text();
    try {
      ...
      throw new Error('محتوای یادداشت‌های دریافتی از Drive خراب یا ناقص بود.');
    }
  }
  ...
  return {
    ensureFolder, uploadBook, listRemoteBooks, downloadBook, checkSyncStatus,
    listRemoteAnnotationFiles, uploadAnnotations, downloadAnnotations,
  };
})();
