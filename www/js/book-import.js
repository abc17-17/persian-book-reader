const BookImport = (() => {
  ...
  async function handleTextImport(file) {
    ...
    const book = { id: generateId(), title, content: html, progress: 0, addedAt: Date.now(), sourceType: 'txt' };
  ...
  async function handlePdfImport(file) {
    ...
    try {
      ...
      const book = { id: generateId(), title, content: fullContent, progress: 0, addedAt: Date.now(), sourceType: 'pdf-ocr', totalPages, failedPages: failed };
      await LocalStore.saveBook(book);
