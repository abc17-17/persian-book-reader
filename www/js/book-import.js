const BookImport = (() => {
  ...
  // ===== پردازشِ پانویس‌ها — جفت‌کردنِ fnref↔fnbody، تخصیصِ id سراسری، بیرون‌کشیدنِ متن =====
  // چرا اینجا (تو finalizeImport، بعد از اتمامِ کاملِ همه‌ی صفحات) نه وسطِ runExtractionLoop:
  // طرح می‌گه جفت‌کردن باید «قبل از join» و «تو محدوده‌ی همون یه صفحه» انجام بشه — چون یه
  // marker مثلِ «۱» می‌تونه تو صفحه‌های مختلف (یا فصل‌های مختلف که هرکدوم از ۱ دوباره شروع
  // می‌کنن) تکراری باشه؛ فقط تو مرزِ یه صفحه بدونِ ابهامه. اگه این کار وسطِ حلقه‌ی استخراج
  // بود، شمارنده‌ی id سراسری باید بینِ pause/resume هم persist می‌شد — پیچیدگیِ غیرضروری.
  // اینجا، دقیقاً مثلِ extractAuthorTranslator، رو متنِ کاملاً آماده کار می‌کنیم: هیچ اثری
  // رو خروجیِ خامِ مدل نداره، فقط بعد از تمومِ OCR روی نتیجه اجرا می‌شه.
  function processFootnotes(pagesDone) {
    const footnotes = {};
    let counter = 1;

    const processedPages = pagesDone.map((pageHtml) => {
      // میان‌بر: اکثرِ صفحات اصلاً پانویس ندارن، برای اون‌ها لازم نیست DOM بسازیم.
      if (!pageHtml || (pageHtml.indexOf('fnref') === -1 && pageHtml.indexOf('fnbody') === -1)) {
        return pageHtml;
      }

      const scratch = document.createElement('div');
      scratch.innerHTML = pageHtml;

      // اول همه‌ی fnbodyهای همین صفحه رو با marker محلی‌شون map کن، بعد کاملاً از جریانِ
      // اصلی حذفشون کن — متنشون جدا نگه داشته می‌شه، نه حتی به‌شکلِ مخفی داخلِ همین HTML
      // (طبقِ طرح: نباید هیچ عنصرِ جدیدی تو .reader-content بمونه که با اندازه‌گیریِ
      // ستون‌ها — تله‌های ۲، ۹، ۱۲ — تداخل کنه).
      const bodiesByMarker = new Map();
      scratch.querySelectorAll('aside.fnbody').forEach((el) => {
        bodiesByMarker.set(el.dataset.fn, el.innerHTML);
        el.remove();
      });

      // بعد هر fnref همین صفحه رو به یه id سراسریِ یکتا (fn-1, fn-2, ...) وصل کن. اگه
      // marker متناظرش تو bodiesByMarker پیدا نشد (یادداشتِ پایانی)، fnref با id سراسری
      // می‌مونه ولی تو footnotes ورودی نداره — reader.js همین نبودن رو موقعِ تپ تشخیص می‌ده.
      scratch.querySelectorAll('sup.fnref').forEach((el) => {
        const marker = el.dataset.fn;
        const globalId = 'fn-' + counter++;
        el.dataset.fn = globalId;
        if (bodiesByMarker.has(marker)) footnotes[globalId] = bodiesByMarker.get(marker);
      });

      return scratch.innerHTML;
    });

    return { processedPages, footnotes };
  }
  ...
  async function handleTextImport(file) {
    ...
    const book = { id: generateId(), title, content: html, progress: 0, addedAt: Date.now(), sourceType: 'txt', author, translator, footnotes: {} };
  ...
  async function finalizeImport(jobId, progress) {
    ...
    // پردازشِ پانویس‌ها قبل از join — هنوز صفحات جدا از همن، پس marker های تکراریِ
    // بینِ صفحات/فصل‌های مختلف قاطی نمی‌شن (نگاه کن به توضیحِ بالای processFootnotes).
    const { processedPages, footnotes } = processFootnotes(progress.pagesDone);
    const fullContent = processedPages.join('\n');
    ...
    const book = {
      ...
      footnotes, // { "fn-1": "متن...", ... } — فیلدِ ساده رو خودِ آبجکتِ کتاب، بدونِ نیاز به DB_VERSION جدید
    };
    await LocalStore.saveBook(book);
