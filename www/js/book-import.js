const BookImport = (() => {
  ...
  async function handlePdfImport(file) {
    ...
    try {
      ...
      const { author, translator } = extractAuthorTranslator(pages.slice(0, 3).join('\n'), title);
      const book = { id: generateId(), title, content: fullContent, progress: 0, addedAt: Date.now(), sourceType: 'pdf-ocr', totalPages, failedPages: failed, author, translator };
      await LocalStore.saveBook(book);
