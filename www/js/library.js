const Library = (() => {
  // آیکونِ ساده و اورجینال (نه از یه آیکون‌فونت خاص) برای نشونِ کارت کتاب.
  ...
  // آیکونِ چرخش برای زمانی که فقط یادداشت‌ها/هایلایت‌ها سینک‌نشده‌ست (نه خودِ کتاب)
  ...
  async function render() {
    ...
    for (const book of books) {
      ...
      // انتخاب آیکون بر اساس اولویتِ نیازِ کتاب — آپلود اگه خودِ کتاب نیست، وگرنه سینک
      let badge = '';
      if (needsBookUpload) {
  ...
  // ===== طاقچه‌ی وارد کردن‌های نصفه‌کاره — بالای کتابخونه، اسکرولِ افقی =====
  // عمداً از گریدِ کتاب‌ها جداست: یه import ناتموم content نداره، اگه قاطیِ کارت‌های
  // معمولی بشه و اشتباهی لمس بشه، تلاش برای بازکردنش تو Reader می‌شکنه.
  async function renderImportShelf() {
    const shelf = document.getElementById('import-shelf');
    if (!shelf) return;
    const jobs = await LocalStore.getAllImportProgress();
    if (!jobs || jobs.length === 0) { shelf.classList.remove('visible'); shelf.innerHTML = ''; return; }

    jobs.sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0));
    shelf.innerHTML = '';
    for (const job of jobs) {
      const done = Math.max(0, (job.nextPageToProcess || 1) - 1);
      const pct = job.totalPages ? Math.round((done / job.totalPages) * 100) : 0;

      const card = document.createElement('div');
      card.className = 'import-shelf-card';
      card.innerHTML = `
        <button class="import-shelf-remove" aria-label="حذف">✕</button>
        <div class="import-shelf-title">${esc(job.title)}</div>
        <div class="import-shelf-pages">${done} از ${job.totalPages} صفحه</div>
        <div class="import-shelf-track"><div class="import-shelf-fill" style="width:${pct}%"></div></div>
      `;
      card.addEventListener('click', () => BookImport.resumeImport(job.id));
      card.querySelector('.import-shelf-remove').addEventListener('click', async (e) => {
        e.stopPropagation(); // نذار کلیک به کارت برسه و resume رو صدا بزنه
        if (!confirm(`وارد کردنِ «${job.title}» (${done} از ${job.totalPages} صفحه) کاملاً حذف بشه؟ این قابل‌بازگشت نیست.`)) return;
        await BookImport.deleteImportJob(job.id);
        renderImportShelf();
      });
    ...
    try {
      ...
      try {
        ...
        if (match) {
          ...
          // همین الان محتوای محلی دقیقاً با Drive یکی شد — صراحتاً ثبتش می‌کنیم، نه
          // اینکه به undefined>undefined=false تصادفی تو محاسبه‌ی needsNotesSync تکیه کنیم.
          book.annotationsSyncedAt = Date.now();
          await LocalStore.saveBook(book);
        }
