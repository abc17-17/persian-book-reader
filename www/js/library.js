const Library = (() => {
  // آیکون‌های ساده و اورجینال (نه از یه آیکون‌فونت خاص) برای دو تا نشونِ کارت کتاب
  const ICON_UPLOAD = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M7.5 17a4.5 4.5 0 0 1-.4-8.98A5.5 5.5 0 0 1 17.5 9.5a4 4 0 0 1-.5 7.98"/><path d="M12 11v7"/><path d="M9 14l3-3 3 3"/></svg>';
  const ICON_SYNC = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 11A8 8 0 0 0 6.3 6.3L4 8.6"/><path d="M4 4v4.6h4.6"/><path d="M4 13a8 8 0 0 0 13.7 4.7L20 15.4"/><path d="M20 20v-4.6h-4.6"/></svg>';
  ...
  async function render() {
    ...
    for (const book of books) {
      ...
      const driveBadge = book.driveFileId ? '' : `<span class="drive-badge" title="پشتیبان‌گیری نشده — لمس کن">${ICON_UPLOAD}</span>`;

      // نشونِ یادداشت‌ها: فقط وقتی این کتاب حداقل یه annotation محلی داره و از
      // آخرین سینکِ یادداشت‌هاش، چیزی (ساخت/ویرایش/حذف/رنگ) تغییر کرده.
      ...
      const notesBadge = needsNotesSync ? `<span class="notes-badge" title="یادداشت/هایلایت سینک‌نشده — لمس کن">${ICON_SYNC}</span>` : '';
      ...
      card.addEventListener('touchstart', (e) => {
        if (e.target.closest('.drive-badge') || e.target.closest('.notes-badge')) return; // بج‌ها خودشون handler جدا دارن، تداخل نکنه
      ...
      const notesBadgeEl = card.querySelector('.notes-badge');
      if (notesBadgeEl) {
        notesBadgeEl.addEventListener('click', (e) => {
          e.stopPropagation();
          syncOneBookNotes(book, notesBadgeEl);
        });
      }
  ...
  // ===== سینکِ یادداشت‌های یک کتابِ خاص — با لمسِ نشونِ سینک رو همون کارت =====
  async function syncOneBookNotes(book, badgeEl) {
    if (!(await Auth.isLoggedIn())) { alert('برای سینک با Drive باید وارد حساب گوگل باشید.'); return; }
    if (!confirm(`هایلایت/یادداشت‌های «${book.title}» تو Google Drive بکاپ بشه؟`)) return;
    ...
    try {
      ...
      statusEl.textContent = 'بکاپِ یادداشت‌ها کامل شد ✓';
      if (badgeEl) badgeEl.remove();
      setTimeout(() => { statusEl.style.display = 'none'; }, 1500);
    } catch (err) {
      statusEl.style.display = 'none';
      alert(`بکاپِ یادداشت‌های «${book.title}» ناموفق بود: ` + err.message);
    }
  }
